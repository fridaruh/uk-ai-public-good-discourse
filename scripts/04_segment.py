"""Fase 2: segmentación en unidades de codificación + detección del término.

Método declarado: escaneo completo → codificación detallada de la sección que
contiene el término. Unidad = sección (bloques contiguos bajo el mismo heading
de nivel superior). Documentos sin término nominal → recuperación de la
reclamación distributiva por lexicón y por embeddings (embeddinggemma local),
marcada con `retrieval` para auditoría.

Actualiza en el manifest: term_status (resuelve los CHECK) y gds_tier
provisional (T1 autoría GDS/CDDO · T2 GDS nombrado en el texto · T3 ausente).
Salidas: coding/units.jsonl · analysis/queries/term_counts.csv
"""
...
SEM_THRESHOLD = 0.52   # cosine mínimo (calibrado a la escala de embeddinggemma
                       # sobre los docs 'absent': los hits correctos caen en 0.52-0.63)
SEM_TOPK = 2           # máx. pasajes semánticos por doc sin término
...
def sections(doc):
    """Agrupa bloques contiguos por heading superior; title queda en la 1a sección."""
...
def main():
    ...
    for r in rows:
        ...
        # recuperación semántica solo donde el lexicón no encontró nada nominal
        if not n_nom:
            secs = sections(doc)
            paras = [(si, head, "\n".join(b["text"] for b in blocks))
        ...
        # documentos cortos sin ninguna unidad → el doc completo es la unidad
        # (cobertura SO3: la comparación intra-familia necesita todos los registros,
        # con o sin término; el genre corto — PRCO/MOU/BLOG/WMS — es una sola pieza retórica)
    ...
    print(f"{len(units)} unidades de codificación "
          f"(lexicón: {sum(1 for u in units if u['retrieval']=='lexicon')}, "
          f"semántica: {sum(1 for u in units if u['retrieval']=='semantic')})")
    ...
    print(f"docs sin término nominal (zero-count): {len(zero)}")
